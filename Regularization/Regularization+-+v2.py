...
def compute_cost_with_regularization(A3, Y, parameters, lambd):
    ...
    L2_regularization_cost = np.dot(lambd/(m*2), (np.sum(np.square(W1)) + np.sum(np.square(W2)) + np.sum(np.square(W3)))   )
    ### END CODER HERE ###
...
def forward_propagation_with_dropout(X, parameters, keep_prob = 0.5):
    ...
    D1[D1 < (keep_prob)] = 1  
    D1[D1 < 1] = 0 # Step 2: convert entries of D1 to 0 or 1 (using keep_prob as the threshold)
    ...
    D2[D2 < (keep_prob)] = 1 
    D2[D2 < 1] = 0 # Step 2: convert entries of D2 to 0 or 1 (using keep_prob as the threshold)
